...
__device__ void gpuCalcEqGrad(double* x, EqInfo* etab, int st, int ed, double* vtab, int* flags)
{
	...
	for (int i = ed - 1 - gthIdx; i >= st; i-= gridSize) 
	{
		const EqInfo eq = etab[i];
	...
	// {
	// 	const EqInfo eq = etab[i];
	// 	if (eq._type == NODE_OPER && etab[eq._left]._op != NODE_OPER && etab[eq._right]._op != NODE_OPER)
	// 	{
	// 		double left = vtab[eq._left];
	// 		double right = vtab[eq._right];
...
int BFGSSolveEqs()
{
	...
	int maxnum = 0;
	for(int i = 1; i < gradEqHeads.size(); ++i)
	{
		if(gradEqHeads[i] - gradEqHeads[i - 1] > maxnum)
